...
import { getSolutions } from 'apiSdk/solutions';
import { SolutionInterface } from 'interfaces/solution';
import { getQuestions } from 'apiSdk/questions';
import { QuestionInterface } from 'interfaces/question';
import { SolutionListPage as ShowCaseList } from 'pages/solutions';

function DashboardPage() {
  const { session } = useSession();

  const solutionStat = useSWR<PaginatedInterface<SolutionInterface>>(
    () => `/solutions`,
    () => getSolutions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults = [
    {
      title: 'Solutions',
      result: solutionStat?.data?.totalCount,
      isLoading: solutionStat.isLoading,
    },
    {
      title: 'Questions',
