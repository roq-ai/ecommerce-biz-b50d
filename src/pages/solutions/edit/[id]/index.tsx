...
import { getSolutionById, updateSolutionById } from 'apiSdk/solutions';
...
function SolutionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<SolutionInterface>(
    () => (id ? `/solutions/${id}` : null),
    () => getSolutionById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SolutionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateSolutionById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<SolutionInterface>({
    initialValues: data,
